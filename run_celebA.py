...
#
# It reproduces the CelebA example from Section 5.
...
def vis():
    ...
    for n in tqdm(range(len(os.listdir(f'{dir_name}/nets')))):
        ...
        if net_num in net_changes and down_factors[factor_num]>1:
            ...
            x_new += .08*torch.randn_like(x_new)
    ut.save_image(x_new,f'{dir_name}/CelebA_samples.png',10)
    exit()
...
if __name__ == '__main__':
    ...
    num_steps = [700000,900000,700000,1900000]
    step_sizes = [.3,.8,.8,1]
    train_steps = 20000
    batch_size = 10
    net_num = 0
    ...
    #load target samples
    data_path = 'celeba'
    celeba = td.CelebA(data_path,transform=transforms.ToTensor(),download=True)
    data = DataLoader(dataset=celeba,batch_size=M)
    data = next(iter(data))[0][...,39:-39,19:-19]
    data = F.interpolate(data,size=[64,64],mode='bicubic').view(M,-1).to(device)
    
    
    p_size = 7 #local size for projections
    p_dim = channel * p_size**2
    step_sizes_scale=[.25,.4,0.8,1,1,1,1,1,1,1]
    scales = [[0],[0],[0,1,2,3],[0,1,2,3,4,5,6]]
    ...
    for factor_num,down_factor in enumerate(down_factors):
        ...
        while True:
                ...
                if cur_size < 32:
                    ...
                    for s in scales[factor_num]:
                        ...
                        for _ in range(proj_batches):
                            ...
                            MMD_grad = s_factor* F.conv1d(xi.reshape([1,1,-1]), grad.unsqueeze(1),
                                            stride=patch_projections).squeeze()/(patch_projections*len(scales[factor_num])) + MMD_grad
            ...
            opt_plus=min(2**step_exp,8196)
            opt_steps=min(opt_steps+opt_plus,100000)
            momentum = min(0.8,momentum + 0.05)
            step_exp+=1    
        ...
        if down_factor>1:
            x_new = F.interpolate(x_new.reshape(M,channel,cur_size,cur_size),scale_factor=2,mode='nearest')
            x_new += .02*torch.randn_like(x_new)
